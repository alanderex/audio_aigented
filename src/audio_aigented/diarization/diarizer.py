...
# Suppress NeMo verbose logging for diarization
...
class NeMoDiarizer:
    ...
    def diarize(self, audio_file: AudioFile) -> List[Tuple[float, float, str]]:
        ...
        Args:
            audio_file: The audio file to diarize. Must be in WAV format.
        ...
        """
        try:
            logger.debug(f"Starting diarization for {audio_file.path.name}")
            ...
            # Validate that audio file is in WAV format
            if audio_file.path.suffix.lower() not in [".wav", ".wave"]:
                raise ValueError(
                    f"Diarization requires WAV format. Got: {audio_file.path.suffix}. "
                    "Please convert to WAV format before diarization."
                )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Suppress verbose output during diarization
                ...
                # Parse results
                speaker_segments = self._parse_results(Path(temp_dir), audio_file)
            ...
            if (
                "ConfigAttributeError" in str(type(e).__name__)
                or "Key" in str(e)
                and "is not in struct" in str(e)
            ):
                logger.error(
                    "This appears to be a configuration error. Please check that all required "
                    "parameters are present in the diarization config file."
                )
                if "smoothing" in str(e):
                    logger.error(
                        "The 'smoothing' parameter should be set to 'false' or a valid smoothing method."
                    )
    ...
    def _prepare_diarization(
        ...
        """
        # Use the audio file path directly
    ...
    def _initialize_diarizer(self, cfg: DictConfig) -> ClusteringDiarizer:
        ...
        """
        logger.debug(
            f"Initializing ClusteringDiarizer with config keys: {list(cfg.keys())}"
        )

        # Suppress verbose output during diarizer initialization
        from ..utils.logging_config import suppress_nemo_output
    ...
    def _parse_results(
        self, temp_dir: Path, audio_file: AudioFile
    ...
    def _find_rttm_file(self, temp_dir: Path, audio_file: AudioFile) -> Optional[Path]:
        """
        ...
        for possible_name in [f"{audio_file.path.stem}.rttm", "input_manifest.rttm"]:

...
@click.option(
    ...
    help="[REQUIRED] Directory containing .wav audio files to process",
)
@click.option(
    "--output-dir",
    "-o",
    type=click.Path(file_okay=False, dir_okay=True, path_type=Path),
    help="Output directory for transcription results (default: ./outputs)",
)
@click.option(
    "--config",
    "-c",
...
@click.option(
    ...
    help="Device for ASR processing: cuda (recommended) or cpu",
)
@click.option(
    "--model-name",
    type=str,
    help="NeMo model: stt_en_conformer_ctc_large (default), nvidia/parakeet-tdt-0.6b-v2 (faster)",
)
@click.option(
...
@click.option(
    "--vocabulary-file",
    type=click.Path(exists=True, file_okay=True, dir_okay=False, path_type=Path),
    help="Custom vocabulary file (.txt) for improved accuracy with domain-specific terms",
)
@click.option(
    "--content-dir",
    multiple=True,
    type=click.Path(exists=True, file_okay=False, dir_okay=True, path_type=Path),
    help="Directory with companion content files (can use multiple times: --content-dir ./docs --content-dir ./notes)",
)
# Output Options
@click.option(
    "--formats",
    type=str,
    help="Output formats: json,txt,attributed (default: all three)",
)
# Utility Options
@click.option(
    "--dry-run",
...
@click.option(
    "--clear-cache",
    is_flag=True,
    help="Clear model cache before processing (or use alone to just clear cache)",
)
@click.option(
    "--create-context-templates",
    is_flag=True,
    help="Create editable context template files for each audio file",
)
...
Supported formats: .wav, .mp3, .m4a, .flac
    ...
    try:
        ...
        if content_dir:
            ...
            pipeline_config.processing["content_directories"] = [
                str(d) for d in content_dir
            ]
            click.echo(
        ...
        if create_context_templates:
            ...
            for file_path in audio_files:
                try:
                    template_path = pipeline.context_manager.save_context_template(
                        file_path
                    )
            ...
            click.echo(
                "Edit these files to provide custom vocabulary, speaker names, and corrections."
            )
            return
